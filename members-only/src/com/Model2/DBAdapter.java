...
public class DBAdapter {
    ...
    public User getUser(String email) {
        try {
            ...
            while(rs.next()) {
                ...
                usr = new User (email, firstName, lastName, 0, null, "member", null);
    ...
    public boolean createPhoto(ImagePost photo) {
        try {
            Connection conn = getConnection();
            int rs = conn.createStatement().executeUpdate("INSERT INTO TrackOneDB.Photo (original) VALUES ('"+photo.path+"')");
        } catch(SQLException e) {
            e.printStackTrace();
            return false;
        }
    ...
    public boolean deleteFilter(Filter filter) {
        try {
            ...
            int rs = conn.createStatement().executeUpdate("DELETE FROM TrackOneDB.FilterPhoto WHERE photoID = " + filter.photoId + "AND filterID =" +filterID);
        }catch(SQLException e) {
            e.printStackTrace();
            return false;
        }
    ...
    */
    ...
    public int[] getFilters(ImagePost photo) {
        ...
        try {
            conn=getConnection();
            ResultSet rs = conn.createStatement().executeQuery("SELECT * FROM TrackOneDB.FilterPhoto WHERE photoID = '"+photo.postID+"'");
