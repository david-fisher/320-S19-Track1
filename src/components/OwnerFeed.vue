      ...
      <div id="feed" style="float:left"  >
        <div v-for="item in jsonData.items1" v-bind:key="item.postID">
          <div class="item">
            ...
            <div>
                <form v-on:click="deletePost($event, item.postID)" >
...
export default {
  ...
  methods: {
    deletePost : function(event, postId) {
        event.preventDefault();
        var jsonObj = {
            email : this.$session.get('email'),
            postID : postId
        }
        const path = this.ip + '/deletePost';
        this.$http.post(path, jsonObj)
        .then(response => {
            console.log(response)
        })
        .catch(error => {
